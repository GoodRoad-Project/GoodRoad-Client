...
class AuthRepositoryTest {
    ...
    fun loginUserReturnsResponse() = runBlocking {
        val api = FakeAuthApi(
            loginResp = AuthResp(user = UserDto(id = "1", role = "USER"))
        )
    ...
    fun registerUserReturnsResponse() = runBlocking {
        val api = FakeAuthApi(
            registerResp = AuthResp(user = UserDto(id = "2", role = "USER"))
        )
        val repository = AuthRepository(api)

        val result = repository.registerUser("Иван", "Петров", "+79990000002", "123")

        assertEquals("2", result.user?.id)
        assertEquals(RegisterReq("Иван", "Петров", "+79990000002", "123"), api.registerReq)
    }
    ...
    fun recoverPasswordReturnsTrueForSuccess() = runBlocking {
        ...
        val result = repository.recoverPassword("+79990000001", "Иван", "Петров", "123")

        assertEquals(true, result)
        assertEquals(RecoverPasswordReq("+79990000001", "Иван", "Петров", "123"), api.recoverReq)
    }
    ...
    fun recoverPasswordThrowsForError() {
        ...
        assertThrows(HttpException::class.java) {
            runBlocking {
                repository.recoverPassword("+79990000001", "Иван", "Петров", "123")
            }
        }
    }
